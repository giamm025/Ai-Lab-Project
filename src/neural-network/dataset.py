...
class SignLanguageDataset(Dataset):
    ...
    def __init__(self, data_dir):
        ...
        self.filenames = []         # conterrà la lista dei file .npy contenenti le coordinate delle mani di ogni video 
        self.labels = []            # conterrà la lista delle soluzioni. questo array è parallelo a filenames (es. se filenames[0] è "hello_12345.npy" allora labels[0] conterrà "0" che tradotto significa "hello")
        ...
        for filename in os.listdir(data_dir):
            ...
            if filename.endswith('.npy'):
                ...
                if word in LABEL_MAP:
                    self.filenames.append(filename)     # salviamo il filename (es. "hello_12345.npy") nella lista dei file
                    self.labels.append(LABEL_MAP[word]) # salviamo la relativa soluzione (es. "0" per "hello")
    ...
    def calculate_max_frames(self, filenames, data_dir):
        ...
        print(f"-> Il video più lungo dura {self.max_frames} frame. Uso questo valore per il padding di tutti i video!")
    ...
    def __getitem__(self, idx):
        ...
        padding = np.zeros((self.max_frames - seq_len, data.shape[1]))  # crea matrici di zeri per i frame mancanti
        data = np.vstack((data, padding))                               # aggiunge gli zeri alla fine dei dati originali
        # ---------------------------------------------------------------------------------------------------------------
